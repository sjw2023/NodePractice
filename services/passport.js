...
passport.use(
  new GoogleStrategy(
    ...
    async (accessToken, refreshToken, profile, done) => {
      const existingUser = await User.findOne({ googleId: profile.id }); // Check if useris already inside of the collection, It returns the promisses
      ...
      if (existingUser) {
        ...
        const user = await new User({ googleId: profile.id }).save(); // this creates the model instance and save into database
        done(null, user); // if save has been done succesfully return( callback )
      }
